...
| Split BLAST / Transcript pages | `HomePage` split into routable `/blast` and `/transcript` under a shared layout route (header + degraded banner). Cross-page flow via URL params: `?job=` restores the selected BLAST job from IndexedDB on back-navigation; `/transcript?db=&id=` / `?db=&chr=&pos=` auto-run lookups and are bookmarkable. Spatial search consolidated on `/transcript` (dedupes two near-identical JSX blocks); BLAST hit buttons navigate instead of mutating local mode state. |
