...
| S3 backend removed | The storage layer's S3/MinIO backend is gone (first step of retiring the orphaned storage subsystem): job artifacts are no longer persisted under the IndexedDB-first architecture, and genome/index data is served via Cloudflare Worker + R2 instead of S3 random reads. `storage.type: s3` now fails validation with a migration hint; the `s3` config section, presigned-URL interface methods, and the minio dependency were deleted. |
