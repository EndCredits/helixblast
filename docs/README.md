...
| Spatial span clustering | Auto-spatial no longer unions the subject span across all HSPs: a low-complexity 2 kb query accumulated noise HSPs megabases apart on the same chromosome, producing an 87 Mb window and a 24,466-feature response. The frontend now anchors on the best HSP and merges only HSPs within a 20 kb gene-scale gap (non-transitive, immune to tandem-repeat chaining); the true locus's gene/mRNA/exon/CDS return as 4 overlapping features. The API rejects windows over 1 Mb with `400` as a server-side guard. |
| Spatial gene-centric semantics | Per design intent, `/api/v1/spatial` answers "which gene is this hit in, and what are the flanking genes": `overlapping` lists **genes** intersecting the range (mRNA/CDS/exon filtered out — they nest under genes and are one Transcript Lookup away), `upstream`/`downstream` are the nearest **genes** (not the nearest any-type feature, which a neighbouring locus's CDS could hijack). JSON field `features` renamed to `overlapping`; both readers filter identically and are cross-checked against a naive oracle (`TestSpatial_GeneOnlyFiltering`, `TestSpatialSearch_CrossCheckRealReaders`). |
